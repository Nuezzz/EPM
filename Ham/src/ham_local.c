...
static inline double FormFactor( unsigned int i, unsigned int j, Lattice *s, double q_norm)
{
	...
	StrCat(&bond, s->mat_name[j]);
     if(!strcmp(bond,"GaAs"))
     {
        Va=(-1.24498*exp(-1.52748*pow((q_norm-0),2))
            +0.0366517*exp(-0.959082*pow((q_norm-2.09782),2))
    ...
    if(!strcmp(bond,"AsGa"))
     {
        Va= (-1.0582*exp(-0.959327*pow((q_norm-0),2))
            -0.00217627*exp(-6.53145*pow((q_norm-2.46808),2))
